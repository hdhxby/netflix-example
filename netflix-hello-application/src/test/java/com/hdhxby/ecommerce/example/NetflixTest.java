...
import com.hdhxby.ecommerce.NetflixHelloApplication;
import static org.junit.Assert.*;

import com.hdhxby.ecommerce.example.feign.WorldClient;
...
public class NetflixTest {
    ...
    public void testIClient() throws Exception {
        ...
        IResponse response = client.execute(new RibbonApacheHttpRequest(context)
                .replaceUri(((RibbonLoadBalancingHttpClient) client).reconstructURIWithServer(
                        ...
                        context.uri()))
                ,null);
        assertEquals("hello world.", new String(((RibbonApacheHttpResponse) response).getInputStream().readAllBytes()));
    }
    ...
    public void testFeign() throws Exception {
        Feign feign = Feign.builder()
                ...
                .build();
        WorldClient worldClient = feign.newInstance(new Target.HardCodedTarget<>(WorldClient.class,"world","http://world"));
        ResponseEntity<String> response = worldClient.world("world",0L);
        assertEquals("hello world.", response.getBody());
    }
    ...
    public void testFeignClient2() throws InterruptedException {
        ...
        AbstractHystrixCommand abstractHystrixCommand = new AbstractHystrixCommand<ResponseEntity<String>>(HystrixCommandBuilder.builder()
            ...
            protected ResponseEntity<String> run() throws Exception {
                latch.countDown();
                return worldClient.world("world",0L);
            }
        };
...
//                ,null);
        ...
        assertEquals("hello world.", new String(response.getBody().readAllBytes()));
    }
}
